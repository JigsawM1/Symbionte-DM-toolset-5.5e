...
## 📡 22. Aprendizajes: Integración de Compendio de Equipo es.json (Julio 2026)

*   **Tipados Discriminados y TypeScript Estático**: Al inyectar propiedades condicionales o extendidas a una unión de tipos Zod (como `ObjetoJuego` que une `Arma`, `Armadura` y `EquipoAventuras`), acceder a propiedades específicas de un subtipo (como `cantidad` en `EquipoAventuras`) en un objeto genérico de la unión arroja errores de compilación estática (`TS2339`).
    *   **Solución**: Usar el operador de discriminación `"propiedad" in objeto` con casteo dinámico `(objeto as any).propiedad` de forma condicional para satisfacer al compilador de TypeScript y asegurar el correcto funcionamiento del build de producción.
*   **Manejo de Ambigüedad de Monedas en Español**: En D&D, las abreviaturas de monedas en inglés (`sp` = silver, `pp` = platinum) y español (`pp` = plata, `ppt` = platino) entran en conflicto directo por el uso de la clave `"pp"`.
    *   **Solución**: Se actualizó el sanitizador de la base de datos para mapear `"pp"` a piezas de plata (dividir entre 10) por defecto al importar compendios traducidos en español, dejando la unidad `"ppt"` o `"plat"` para la conversión a piezas de platino (multiplicar por 10), previniendo que los costos en plata de armas/equipo estándar se inflen accidentalmente.
*   **Navegación Histórica Interactiva en Overlays**: La incorporación de navegación relacional en la UI de detalles del Simbionte (como el crafteo de herramientas o el contenido de paquetes) requiere almacenar la ruta del usuario para evitar una experiencia bloqueante.
    *   **Solución**: Implementamos un stack de historial local (`historialDetalle`) en el estado del componente React. Esto permite almacenar la ID de la ficha anterior al saltar a un sub-objeto o ingrediente de receta y restaurar la vista anterior mediante un botón dinámico "Atrás", logrando una experiencia de navegación fluida.
