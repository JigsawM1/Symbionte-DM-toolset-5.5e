...
import {
  ...
  reiniciarAlmacenamientoCompleto
...
/**
 * Calculates a monster's maximum and current HP based on the DM's preference.
 * Supports D&D hit dice formulas (e.g. "3d8+6", "8d10 + 16", "22 (4d8+4)") and
 * strictly bounds the random output within the mathematical possible range.
 */
export function calcularVidaPorPreferencia(
  vidaMaximaDefecto: number,
  vidaNotas?: string,
  preferencia: "bloque" | "maximo" | "aleatorio" = "bloque"
): number {
  if (preferencia === "bloque" || !vidaNotas) {
    return vidaMaximaDefecto;
  }

  // Buscamos la fórmula de dados en notas de vida (ej. "3d8+6" o "8d10 - 2")
  const matchFormula = vidaNotas.match(/(\d+)\s*d\s*(\d+)(?:\s*([+-])\s*(\d+))?/i);
  if (!matchFormula) {
    return vidaMaximaDefecto;
  }

  const cantidad = parseInt(matchFormula[1], 10);
  const caras = parseInt(matchFormula[2], 10);
  const signo = matchFormula[3] || "+";
  const modificador = matchFormula[4] ? parseInt(matchFormula[4], 10) : 0;

  if (isNaN(cantidad) || isNaN(caras)) {
    return vidaMaximaDefecto;
  }

  const modEfectivo = signo === "-" ? -modificador : modificador;

  if (preferencia === "maximo") {
    return (cantidad * caras) + modEfectivo;
  }

  if (preferencia === "aleatorio") {
    let suma = 0;
    for (let i = 0; i < cantidad; i++) {
      suma += Math.floor(Math.random() * caras) + 1;
    }
    const resultado = suma + modEfectivo;
    // Rango matemático estricto según reglamento de D&D
    const minPosible = cantidad + modEfectivo;
    const maxPosible = (cantidad * caras) + modEfectivo;
    return Math.max(1, Math.min(maxPosible, Math.max(minPosible, resultado)));
  }
...
export interface EstadoDM {
  ...
  esGM: boolean;
  preferenciaVidaMonstruo: "bloque" | "maximo" | "aleatorio";
  // Flag de seguridad: previene que la sincronización de TaleSpire sobreescriba
  // los datos persistidos antes de que cargarDatosPersistidos termine.
  ...
  establecerPreferenciaVidaMonstruo: (preferencia: "bloque" | "maximo" | "aleatorio") => void;
  ...
  actualizarIniciativaCriatura: (id: string, iniciativa: number) => void;
  ...
  // Carga e Importación
  cargarDatosPersistidos: () => Promise<void>;
...
const guardarEstadoCombateLocal = (cola: CriaturaIniciativa[], ronda: number, turno: number) => {
  // inmediato=true: escribe al blob de TaleSpire sin debounce,
  // evitando pérdida de datos si el usuario cierra el symbiote.
  guardarDatoFragmentado("dm_cola_iniciativa", cola, true);
  guardarDatoFragmentado("dm_ronda_actual", String(ronda), true);
  guardarDatoFragmentado("dm_indice_turno_activo", String(turno), true);
};
...
export const usarAlmacenDM = create<EstadoDM>((set, get) => ({
  ...
  esGM: true,
  preferenciaVidaMonstruo: "bloque",
  datosCargados: false, // Se activa cuando cargarDatosPersistidos termina
  ...
  establecerPreferenciaVidaMonstruo: (preferencia) => {
    guardarDatoFragmentado("dm_preferencia_vida", preferencia);
    set({ preferenciaVidaMonstruo: preferencia });
  },
  ...
  actualizarColaIniciativaDesdeTaleSpire: (colaTS) => set((state) => {
    // PROTECCIÓN CRÍTICA: Ignorar sincronización hasta que los datos persitidos
    // hayan cargado completamente. Esto previene el bug de borrado de datos al refrescar.
    if (!state.datosCargados) {
      console.log("[TaleSpire Simbionte] Sincronización ignorada: datos no cargados aún.");
      return {};
    }
    ...
    const nuevasCriaturasNativas = colaTS.map((cTS) => {
      ...
      // Si no existe, buscamos por nombre en la base de datos de monstruos del DM
      const plantillaMonstruo = state.baseDatosMonstruos.find(
        (m) => m.nombre.toLowerCase() === cTS.name.toLowerCase()
      );

      const vidaMaxCalculada = plantillaMonstruo 
        ? calcularVidaPorPreferencia(plantillaMonstruo.vidaMaxima, plantillaMonstruo.vidaNotas, state.preferenciaVidaMonstruo)
        : 10;
  ...
  modificarVidaCriaturaIniciativa: (id, nuevaVida) => set((state) => {
    const nuevaCola = state.colaIniciativa.map((c) => {
      if (c.id === id) {
        const actual = c.vidaActual;
        const dif = nuevaVida - actual; // Si dif es negativo, es daño
        if (dif < 0) {
          const daño = Math.abs(dif);
          let vidaTempActual = c.vidaTemporal || 0;
          let dañoRestante = daño;
          
          if (vidaTempActual > 0) {
            if (vidaTempActual >= dañoRestante) {
              vidaTempActual -= dañoRestante;
              dañoRestante = 0;
            } else {
              dañoRestante -= vidaTempActual;
              vidaTempActual = 0;
            }
          }
          const nuevaVidaActual = Math.max(0, actual - dañoRestante);
          return { ...c, vidaActual: nuevaVidaActual, vidaTemporal: vidaTempActual };
        } else {
          // Si es cura (dif > 0) o no hay cambio
  ...
  actualizarIniciativaCriatura: (id, iniciativa) => set((state) => {
    const nuevaCola = state.colaIniciativa.map((c) => {
      if (c.id === id) {
        return { ...c, iniciativa };
      }
  ...
  guardarNotasDM: (notas) => set(() => {
    guardarDatoFragmentado("dm_notas", notas);
  ...
  cargarDatosPersistidos: async () => {
    // Inicializar el almacenamiento unificado y cargar datos persistidos (blobs)
    ...
    // 5. Cargar Notas
    const notas = obtenerDatoFragmentado<string>("dm_notas");
    if (notas !== null) {
      set({ notasDM: notas });
    }
    ...
    const rondaGuardada = obtenerDatoFragmentado<string>("dm_ronda_actual");
    if (rondaGuardada) {
      set({ rondaActual: Number(rondaGuardada) || 1 });
    }
    const turnoGuardado = obtenerDatoFragmentado<string>("dm_indice_turno_activo");
    if (turnoGuardado) {
      set({ indiceTurnoActivo: Number(turnoGuardado) || 0 });
    }

    // 8. Cargar Preferencia de Vida para Monstruos
    const prefVida = obtenerDatoFragmentado<string>("dm_preferencia_vida");
    if (prefVida === "bloque" || prefVida === "maximo" || prefVida === "aleatorio") {
      set({ preferenciaVidaMonstruo: prefVida as "bloque" | "maximo" | "aleatorio" });
    }

    // ✅ Marcar como cargado: a partir de aquí, actualizarColaIniciativaDesdeTaleSpire
    // puede operar sin riesgo de sobrescribir datos del usuario.
    set({ datosCargados: true });
    console.log("[TaleSpire Simbionte] Datos persistidos cargados correctamente. Sincronización de iniciativa habilitada.");
  },
  ...
  importarBaseDatosJSONCompleta: (datosJSON) => {
    try {
      ...
      if (monstruosCandidatos.length > 0) {
        ...
        guardarDatoFragmentado("dm_monstruos_homebrew", homebrewNeto, true);
        modificado = true;
      }
      ...
      if (hechizosCandidatos.length > 0) {
        ...
        guardarDatoFragmentado("dm_hechizos_homebrew", homebrewNeto, true);
        modificado = true;
      }
      ...
      if (objetosCandidatos.length > 0) {
        ...
        guardarDatoFragmentado("dm_objetos_homebrew", combinados, true);
        modificado = true;
      }
